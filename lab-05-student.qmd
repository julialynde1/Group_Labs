...
1.  What sign do you expect for the correlation between `x` and `y`?
    ...
    A positive sign, since this correlation shows a positive and strong correlation.\
...
-   Interpret the slope: what does a one-unit increase in $X$ imply for $Y$?\
    **A one-unit increase in X implies a 1-point increase in Y.**

-   How close are your manual and R estimates? Why are they identical (up to rounding)?

    **My manual and R estimates are identical because** **we are computing exactly the same quantities that R is computing. Linear regression has closed-form solutions, meaning the results are very close. We are using the same quantities and same formulas.**
...
-   What happens to predicted earnings when education increases by 2 years?\
    **Predicted earnings increase by \$203.10 when education increases by 2 years between 12 and 14 years.**

-   Can we interpret this as a **causal effect** of education on income? Why or why not?\
    **We cannot interpret this as a causal effect because the condition E\[U\|X\]=0 must hold. U could be any external factor that affects income, which can affect a direct causal outcome.**

-   What omitted factors might bias the estimate?

    **Factors like physical and mental health, access to resources, and other can bias the estimate.**
...
The newly generated slope is 2.5, so it does not recover the true value 2. This is because I changed the error term to be U=0.5x. It is now correlated with x. So, I unintentionally added an extra 0.5x into the model, making the slope 2.5 instead of 2. This violates the OLS assumption that the error term must be independent of the regressor.
...
ggplot(simdata, aes(x=x, y=y)) +
  geom_point(color="grey40") +
  geom_smooth(method="lm") +
  labs(title="Simulated Data: Y= 5 + 2X + U",
       x="X", y="Y")
```
